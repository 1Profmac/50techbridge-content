# LMT Social Accounts — Single Source of Truth
## Last Updated: 2026-03-23

---

## Active Business Accounts

| Platform | Account Name | URL | Followers | Status |
|----------|-------------|-----|-----------|--------|
| LinkedIn (Company) | Learn More Technologies, LLC | linkedin.com/company/learn-more-technologies-llc | 2 | Set up, privacy locked |
| LinkedIn (Personal) | Brian McKinney | linkedin.com/in/brianmckinneylmt | 164 | Privacy locked, ad data off |
| Facebook (Business) | Learn More Technologies | facebook.com/learnmoretechnologies | 859 | Set up, branded |
| Facebook (Personal) | Brian McKinney | facebook.com/mckinneymedia1 | 62 friends | Privacy locked, banner updated |
| Facebook (Page 2) | 50Plustechbridge | TBD | TBD | Needs audit |
| Instagram | @50plustechbridge | instagram.com/50plustechbridge | 10 | Renamed, needs full setup |
| YouTube | Learn More Technologies @LearnMoreTechnologies | youtube.com/c/LearnMoreTechnologies | 517 | Fully set up, branded, links updated |
| X / Twitter | TBD | TBD | TBD | Not started |

---

## Accounts to Delete

| Platform | Account | Reason |
|----------|---------|--------|
| Instagram | @smallbusinessonlinesalescoach | Old branding, 67 followers, 0 posts |
| Instagram | @mckinney_media | Old branding (if it still exists) |

---

## Email Structure

| Email | Role |
|-------|------|
| brian@learnmoretechnologies.com | Business only — clients, partners, social accounts |
| brian@learnmo.com | Personal + admin |
| b@learnmo.com | Kill — forward to brian@learnmo.com |

---

## Planned Tools

| Tool | Purpose | Status |
|------|---------|--------|
| Metricool | Analytics — connect all 5 platforms | NOT CONNECTED |
| Buffer | Scheduling (SAGE agent) | NOT CONNECTED |
| Brand24 | Mention monitoring | NOT CONNECTED |
